...
mkdir boxplot_total_area

final_result = [];
for BWthreshold = 0.86
    disp(['Threshold:, ' num2str(BWthreshold)])
    for noise_threshold = 4
...
%T_1 is to enlarge objects
    ...
    properties_data(ite, 7) = sum(area_3);
...
plot(properties_data(:, 4))
mkdir Results_particles_number;
fig_name = ['/Results_particles_number/', num2str(BWthreshold), '_', num2str(noise_threshold) '.jpg'];
...
properties_data(1,7) = {'particle total area for BW_3'};

data = cell2mat(properties_data(2:16, 1:7));
group = repmat(1:3, [5, 1]);
group = group(:)';
group = group';
data = [group, data];
figure("visible", "off");
boxplot(data(:, 8), data(:, 1))
file_boxplot_name = ['/boxplot_total_area/', num2str(BWthreshold), '_', num2str(noise_threshold), '.jpg'];
saveas(gcf, [pwd, file_boxplot_name])
close;
